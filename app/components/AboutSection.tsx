...
//     <section
//       id='about'
//       className='min-h-screen flex flex-col md:flex-row items-center justify-center gap-12 py-12 px-6 md:px-16 bg-background-light dark:bg-background-dark text-gray-900 dark:text-gray-100 transition-colors duration-500'
//     >
//       {/* IMAGE SIDE */}
//       <motion.div
...
export default function AboutSection() {
  // Spring config for general animations
  const spring = {
    type: 'spring',
    damping: 10,
    stiffness: 100,
  ...
  const floatTransition: any = {
    y: {
      duration: 3,
      repeat: Infinity,
      repeatType: 'reverse', // ✅ correct type
    },
    default: spring, // apply spring to other properties
  }
  ...
  return (
    ...
    >
      ...
      <motion.div
        ...
        whileInView={{ opacity: 1, y: [0, -15, 0] }} // floating effect
        transition={floatTransition} // ✅ single transition prop
        className='relative w-64 h-64 md:w-80 md:h-80 rounded-2xl overflow-hidden shadow-xl order-0 md:order-0 mb-6 md:mb-0'
      >
        <Image
          src={avartar}
          alt='About the Designer'
      ...
      <motion.div
        ...
        viewport={{ once: true }}
        className='max-w-xl text-center md:text-left order-1 md:order-1'
      >
        <h2 className='text-3xl md:text-5xl font-bold font-poppins text-primary mb-6'>
          About Me 💫
        </h2>
